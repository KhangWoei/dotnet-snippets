...
internal sealed class UriDiscoveredNotificationHandler(
    IVisitationPolicy policy,
    ICrawlSourceFactory factory,
    ISeedQueue<Task<ICrawlSource>> seedQueue,
    Configuration configuration) : INotificationHandler<UriDiscoveredNotification>
{
    ...
    {
        if (Seen.TryAdd(notification.Seed.Host, 1) 
            && await policy.ShouldVisit(notification.Seed, cancellationToken)
            && Interlocked.Increment(ref _currentWidth) <= configuration.Width)
        {
            var createCrawlSourceTask =
                factory.Create(notification.Seed.ToString(), notification.Depth, cancellationToken);
